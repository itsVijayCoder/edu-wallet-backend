...
func TestBillingFeeSetupInvoiceLedger_E2E(t *testing.T) {
	...
	parentUserID := SeedUser(t, suite.Pool, suite.Hasher, "billing-parent@example.com", "password123", []string{"parents"})
	...
	parentToken, err := suite.TokenMgr.GenerateTenantAccess(parentUserID, "billing-parent@example.com", []string{"parents"}, tenantID, nil)
	...
	w = doRequest(suite.Server, http.MethodGet, "/api/v1/parent/children?search=Aar&page=1&page_size=1", nil, bearer(parentToken))
	require.Equal(t, http.StatusOK, w.Code, w.Body.String())
	body = parseJSON(t, w)
	children := body["data"].(map[string]any)
	rows := children["rows"].([]any)
	require.Len(t, rows, 1)
	assert.Equal(t, studentID.String(), rows[0].(map[string]any)["id"])
	assert.Equal(t, "Class 10", rows[0].(map[string]any)["class_name"])
	meta := children["meta"].(map[string]any)
	...
	require.Len(t, dues["invoices"].([]any), 1)

	otherParentID := SeedUser(t, suite.Pool, suite.Hasher, "other-billing-parent@example.com", "password123", []string{"parents"})
	otherParentToken, err := suite.TokenMgr.GenerateTenantAccess(otherParentID, "other-billing-parent@example.com", []string{"parents"}, tenantID, nil)
	require.NoError(t, err)
	w = doRequest(suite.Server, http.MethodGet, "/api/v1/parent/children/"+studentID.String()+"/dues", nil, bearer(otherParentToken))
